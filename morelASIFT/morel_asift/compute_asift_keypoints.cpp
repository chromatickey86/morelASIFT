...
int compute_asift_keypoints(vector<float>& image, int width, int height, int num_of_tilts, int verb,
									 std::vector<std::vector<keypointslist>>& keypoints,
...
// keypoints (output): ASIFT keypoints. It is a 2D matrix with varying rows and columns. Each entry keypoints[tt][rr]
...
{	
  ...
  /* Calculate the number of simulations, and initialize keypoints */
	keypoints = std::vector<std::vector<keypointslist>>(num_tilt);
  ...
  for (tt = 1; tt <= num_tilt; tt++)
  {
    ...
    if ( t == 1 )
    {
      ...
      keypoints[tt-1] = std::vector<keypointslist>(1);
    }
    else
    {
      int num_rot1 = (int)round(num_rot_t2 * t/2);
      if ( num_rot1%2 == 1 )
      {
        num_rot1 = num_rot1 + 1;
      }
      ...
      keypoints[tt-1] = std::vector<keypointslist>(num_rot1);
    }         		
  }
...
#pragma omp parallel for private(tt)
  for (tt = 1; tt <= num_tilt; tt++)
  {
    ...
    {					
      ...
      compute_sift_keypoints(image_tmp1_float, keypoints[tt-1][0], width,height, siftparameters);
...
#pragma omp parallel for private(rr)
      for ( int rr = 1; rr <= num_rot1; rr++ ) 
      {
        ...
        keypointslist tkeypoints;
        keypointslist tkeypoints_filtered;
        compute_sift_keypoints(image_tmp1_float, tkeypoints, width_t, height_t, siftparameters);
		  for(int itKps=0; itKps<(int)tkeypoints.size(); itKps++) {
			  tkeypoints[itKps].tilt = t;
			  tkeypoints[itKps].longitude = theta;
		  }
        ...
        if ( keypoints.size() != 0 )
        {
          ...
          {		      
            ...
            {				 					   
              ...
              tkeypoints_filtered.push_back(tkeypoints[cc]);
            }				   
          }
        }			 
        keypoints[tt-1][rr-1] = tkeypoints_filtered;
      }		 
    }         
  }

  {		
    for (tt = 0; tt < (int) keypoints.size(); tt++)
      for (rr = 0; rr < (int) keypoints[tt].size(); rr++)
      {
        num_keys_total += (int) keypoints[tt][rr].size();
      }				
